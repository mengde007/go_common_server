...
//初始化加锁客户端
func init() {
	aServerHost := common.ReadServerClientConfig("matchdaerserver")
	if len(aServerHost) == 0 {
		logger.Fatal("load config failed, no server")
		return
	}

	pPoll = rpcplusclientpool.CreateClientPool(aServerHost, "matchdaerserver")
	if pPoll == nil {
...
// //创建房间
// func CreateRoomREQ(base *rpc.PlayerBaseInfo, client *rpc.CreateRoomREQ) error {
// 	err, conn := pPoll.RandomGetConn()
// 	if err != nil {
// 		return err
// 	}
