...
func (call *Call) done() {
	...
	default:
		...
		logger.Error("rpc: discarding Call reply due to insufficient Done chan capacity")
	}
}
...
func (client *Client) Go(serviceMethod string, args interface{}, reply interface{}, done chan *Call) *Call {
	...
	if done == nil {
		...
		if cap(done) == 0 {
			logger.Fatal("rpc: done channel is unbuffered")
		}
	}
	call.Done = done
	client.send(call)
...
func (client *Client) StreamGo(serviceMethod string, args interface{}, replyStream interface{}) *Call {
	...
	if typ.Kind() != reflect.Chan || typ.Elem().Kind() != reflect.Ptr {
		logger.Fatal("rpc: replyStream is not a channel of pointers")
		return nil
	}
