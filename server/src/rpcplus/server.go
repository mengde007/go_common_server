...
func prepareMethod(method reflect.Method) *methodType {
	...
	if replyType.Kind() == reflect.Func {
		...
		if replyType.NumIn() != 1 {
			logger.Error("method", mname, "sendReply has wrong number of ins:", replyType.NumIn())
			return nil
		}
		if replyType.In(0).Kind() != reflect.Interface {
			logger.Error("method", mname, "sendReply parameter type not an interface:", replyType.In(0))
			return nil
		}
		if replyType.NumOut() != 1 {
			logger.Error("method", mname, "sendReply has wrong number of outs:", replyType.NumOut())
			return nil
		}
		if returnType := replyType.Out(0); returnType != typeOfError {
			logger.Error("method", mname, "sendReply returns", returnType.String(), "not error")
			return nil
		}
...
func (server *Server) sendResponse(sending *sync.Mutex, req *Request, reply interface{}, codec ServerCodec, errmsg string, last bool) (err error) {
	...
	if err != nil {
		logger.Error("rpc: writing response:", err)
	}
	sending.Unlock()
...
func (s *service) call(server *Server, sending *sync.Mutex, mtype *methodType, req *Request, argv, replyv reflect.Value, codec ServerCodec, context interface{}) {
		...
		func() {
			defer func() {
				if r := recover(); r != nil {
					...
					logger.Info("rpcplus call error: %s", r)
					sysdebug.PrintStack()
				}
			}()
	...
	sendReply := func(oneReply interface{}) error {
		...
		if firstType == nil {
			...
			if firstType != typ {
				logger.Error("passing wrong type to sendReply",
					firstType, "!=", typ)
	...
	func() {
		defer func() {
			if r := recover(); r != nil {
				err := fmt.Sprintf("rpcplus call error :%s", r)
				returnValues = append(returnValues, reflect.ValueOf(errors.New(err)))
				logger.Error("rpcplus call error: %s", r)
				sysdebug.PrintStack()
			}
		}()
...
func (server *Server) Accept(lis net.Listener) {
	for {
		conn, err := lis.Accept()
		if err != nil {
			logger.Fatal("rpc.Serve: accept:", err.Error()) // TODO(r): exit?
		}
...
func (server *Server) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		logger.Error("rpc hijacking ", req.RemoteAddr, ": ", err.Error())
		return
	}
