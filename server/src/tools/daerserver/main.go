...
func main() {
	var cfg common.DaerConfig
	if err := common.ReadDaerServerConfig(&cfg); err != nil {
		logger.Error("load config failed, error is: %v", err)
		return
	}

	common.DebugInit(cfg.GcTime, cfg.DebugHost, "daerserver", cfg.CpuProfile)
	...
	go daerserver.CreateServices(cfg, listener)
